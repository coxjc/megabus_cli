...
USER_AGENT_STR = 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_14_4) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/78.0.3904.97 Safari/537.36'
...
def get_origin_cities():
    """Get origin cities."""
    r = requests.get(ORIGIN_CITIES_URL, headers=HEADERS)
    try:
...
def get_destination_cities(origin_city_id):
    r = requests.get(DEST_CITIES_URL(origin_city_id), headers=HEADERS)
    try:
        return r.json()["cities"]
    except Exception as e:
        raise e
...
def get_dates(origin_city_id, destination_city_id):
    r = requests.get(DATES_URL(origin_city_id, destination_city_id), headers=HEADERS)
    try:
